...
(function (Utils) {
    ...
    function createDiagnosticMessageReplacer(diagnosticMessage, replacer) {
        ...
        var regExp = new RegExp("^(?:" + messageParts.map(ts.regExpEscape).join("(.*?)") + ")$");
        return function (text) {
            ...
            return text.replace(regExp, function (_) {
                ...
                return ts.formatStringFromArgs(diagnosticMessage.message, replacer.apply(void 0, __spreadArray([fixedArgs], args)));
            });
        };
    }
    ...
    function theory(name, cb, data) {
        var _loop_1 = function (entry) {
            it(name + "(" + entry.map(formatTheoryDatum).join(", ") + ")", function () { return cb.apply(void 0, entry); });
        };
    ...
    var FileSystem = /** @class */ (function () {
        ...
        FileSystem.trackCreatedInodes = function (container, changed, changedLinks) {
            ...
            if (!changedLinks.size)
                return false;
            changedLinks.forEach(function (node, basename) { FileSystem.trackCreatedInode(container, basename, changed, node); });
            return true;
        };
        FileSystem.trackDeletedInodes = function (container, baseLinks) {
            ...
            if (!baseLinks.size)
                return false;
            baseLinks.forEach(function (node, basename) { container[basename] = isDirectory(node) ? new Rmdir() : new Unlink(); });
            return true;
        };
...
(function (compiler) {
    ...
    function compileFiles(host, rootFiles, compilerOptions) {
        ...
        var preProgram = ts.length(rootFiles) < 100 ? ts.createProgram(rootFiles || [], __assign(__assign({}, compilerOptions), { configFile: compilerOptions.configFile, traceResolution: false }), host) : undefined;
        ...
        var errors = preErrors && (preErrors.length !== postErrors.length) ? __spreadArray(__spreadArray([], postErrors), [ts.addRelatedInfo.apply(ts, __spreadArray([ts.createCompilerDiagnostic({
                ...
                })], ts.filter(postErrors, function (p) { return !ts.some(preErrors, function (p2) { return ts.compareDiagnostics(p, p2) === 0 /* EqualTo */; }); })))]) : postErrors;
...
(function (evaluator) {
    ...
    function createLoader(fs, globals) {
        var moduleCache = new ts.Map();
        return load;
        function evaluate(text, file, module) {
            var globalNames = [];
            var globalArgs = [];
            ...
            var localRequire = function (id) { return requireModule(id, base); };
            var evaluateText = "(function (module, exports, require, __dirname, __filename, " + globalNames.join(", ") + ") { " + text + " })";
            // eslint-disable-next-line no-eval
            var evaluateThunk = (void 0, eval)(evaluateText);
            evaluateThunk.call.apply(evaluateThunk, __spreadArray([globals, module, module.exports, localRequire, vpath.dirname(file), file, FakeSymbol], globalArgs));
        }
        function loadModule(file) {
            if (!ts.isExternalModuleNameRelative(file))
                throw new Error("Module '" + file + "' could not be found.");
            var module = moduleCache.get(file);
            if (module)
                return module;
            moduleCache.set(file, module = { exports: {} });
            try {
                var sourceText = fs.readFileSync(file, "utf8");
...
(function (fakes) {
    ...
    function expectedErrorDiagnosticToText(_a) {
        ...
        var text = DiagnosticKind.Error + "!: " + expectedDiagnosticRelatedInformationToText(diagnosticRelatedInformation);
    ...
    function diagnosticToText(_a) {
        ...
        var text = kind + "!: " + diagnosticRelatedInformationToText(diagnosticRelatedInformation);
        if (relatedInformation) {
    ...
    function patchHostForBuildInfoReadWrite(sys) {
        ...
        sys.readFile = function (path, encoding) {
            ...
            if (!value || !ts.isBuildInfoFile(path))
                return value;
            var buildInfo = ts.getBuildInfo(value);
            ts.Debug.assert(buildInfo.version === fakes.version);
            buildInfo.version = ts.version;
            return ts.getBuildInfoText(buildInfo);
        };
        var originalWrite = sys.write;
        sys.write = function (msg) { return originalWrite.call(sys, msg.replace(ts.version, fakes.version)); };
        if (sys.writeFile) {
            ...
            sys.writeFile = function (fileName, content, writeByteOrderMark) {
                if (!ts.isBuildInfoFile(fileName))
                    return originalWriteFile_1.call(sys, fileName, content, writeByteOrderMark);
                var buildInfo = ts.getBuildInfo(content);
                buildInfo.version = fakes.version;
                originalWriteFile_1.call(sys, fileName, ts.getBuildInfoText(buildInfo), writeByteOrderMark);
            };
        }
        return sys;
    }
    ...
    (function (server) {
        ...
        var SessionClient = /** @class */ (function () {
                ...
                return {
                    ...
                    entries: response.body.map(function (entry) {
                        if (entry.replacementSpan !== undefined) {
                            var name = entry.name, kind = entry.kind, kindModifiers = entry.kindModifiers, sortText = entry.sortText, replacementSpan = entry.replacementSpan, hasAction = entry.hasAction, source = entry.source, data = entry.data, isRecommended = entry.isRecommended;
                            // TODO: GH#241
                            var res = { name: name, kind: kind, kindModifiers: kindModifiers, sortText: sortText, replacementSpan: _this.decodeSpan(replacementSpan, fileName), hasAction: hasAction, source: source, data: data, isRecommended: isRecommended };
                            return res;
                        }
            ...
            SessionClient.prototype.getEmitOutput = function (file) {
                var request = this.processRequest("emit-output" /* EmitOutput */, { file: file });
                var response = this.processResponse(request);
                return response.body;
            };
            ...
            SessionClient.prototype.getEncodedSemanticClassifications = function (file, span, format) {
                var request = this.processRequest("encodedSemanticClassifications-full" /* EncodedSemanticClassificationsFull */, { file: file, start: span.start, length: span.length, format: format });
...
(function (Utils) {
            ...
            for (var _i = 0, _a = Object.getOwnPropertyNames(n); _i < _a.length; _i++) {
                ...
                switch (propertyName) {
                    ...
                    case "flags":
                        ...
                        // on the AST.
                        var flags = n.flags & ~(131072 /* JavaScriptFile */ | 524288 /* HasAggregatedChildData */);
                        if (flags) {
    ...
    function assertStructuralEquals(node1, node2) {
        ...
        assert.equal(node1.flags & ~2816 /* ReachabilityAndEmitFlags */, node2.flags & ~2816 /* ReachabilityAndEmitFlags */, "node1.flags !== node2.flags");
...
(function (Harness) {
    ...
    var tcServicesFileName = ts.combinePaths(Harness.libFolder, "typescriptServices.js");
    Harness.tcServicesFile = Harness.IO.readFile(tcServicesFileName) + Harness.IO.newLine() + ("//# sourceURL=" + Harness.IO.resolvePath(tcServicesFileName));
    // Settings
    /* eslint-disable prefer-const */
    ...
    (function (Compiler) {
        ...
        function getDefaultLibrarySourceFile(fileName) {
            ...
            if (!libFileNameSourceFileMap) {
                libFileNameSourceFileMap = new ts.Map(ts.getEntries((_a = {},
                    _a[Compiler.defaultLibFileName] = createSourceFileAndAssertInvariants(Compiler.defaultLibFileName, Harness.IO.readFile(Harness.libFolder + "lib.es5.d.ts"), /*languageVersion*/ 99 /* Latest */),
                    _a)));
            }
            var sourceFile = libFileNameSourceFileMap.get(fileName);
            if (!sourceFile) {
                libFileNameSourceFileMap.set(fileName, sourceFile = createSourceFileAndAssertInvariants(fileName, Harness.IO.readFile(Harness.libFolder + fileName), 99 /* Latest */));
            }
            return sourceFile;
        }
            ...
            function outputErrorText(error) {
                ...
                if (error.relatedInformation) {
                    for (var _i = 0, _a = error.relatedInformation; _i < _a.length; _i++) {
                        var info = _a[_i];
                        errLines.push("!!! related TS" + info.code + (info.file ? " " + ts.formatLocation(info.file, info.start, formatDiagnsoticHost, ts.identity) : "") + ": " + ts.flattenDiagnosticMessageText(info.messageText, Harness.IO.newLine()));
                    }
                }
        ...
        function doTypeAndSymbolBaseline(baselinePath, program, allFiles, opts, multifile, skipTypeBaselines, skipSymbolBaselines, hasErrorBaseline) {
            ...
            var fullWalker = new Harness.TypeWriterWalker(program, /*fullTypeCheck*/ true, !!hasErrorBaseline);
            ...
            function iterateBaseLine(isSymbolBaseline, skipBaseline) {
                var dupeCase, _i, allFiles_1, file, unitName, typeLines, codeLines, gen, lastIndexWritten, _a, done, result, typeOrSymbolString, formattedLine, _b, codeLines_1, codeLine;
                var _c;
                return __generator(this, function (_d) {
                    switch (_d.label) {
                        ...
                        case 1:
                            ...
                            for (_a = gen.next(), done = _a.done, result = _a.value; !done; _c = gen.next(), done = _c.done, result = _c.value) {
                            ...
                            if (lastIndexWritten === undefined) {
                                for (_b = 0, codeLines_1 = codeLines; _b < codeLines_1.length; _b++) {
                                    codeLine = codeLines_1[_b];
        ...
        function doJsEmitBaseline(baselinePath, header, options, result, tsConfigFiles, toBeCompiled, otherFiles, harnessSettings) {
            ...
            result.js.forEach(function (file) {
                ...
                if (!result.diagnostics.length && !ts.endsWith(file.file, ".json" /* Json */)) {
                    var fileParseResult = ts.createSourceFile(file.file, file.text, options.target || 0 /* ES3 */, /*parentNodes*/ false, ts.endsWith(file.file, "x") ? 2 /* JSX */ : 1 /* JS */);
            ...
            // eslint-disable-next-line no-null/no-null
            Baseline.runBaseline(baselinePath.replace(/\.tsx?/, ".js" /* Js */), jsCode.length > 0 ? tsCode + "\r\n\r\n" + jsCode : null);
        }
            ...
            if (ts.hasProperty(settings, varyByKey)) {
                ...
                if (entries) {
                    ...
                    if (variationCount > 25)
                        throw new Error("Provided test options exceeded the maximum number of variations: " + varyBy.map(function (v) { return "'@" + v + "'"; }).join(", "));
...
(function (ts) {
    ...
    (function (TestFSWithWatch) {
        ...
        function createServerHost(fileOrFolderList, params) {
            var host = new TestServerHost(/*withSafelist*/ true, fileOrFolderList, params);
        ...
        function checkMap(caption, actual, expectedKeysMapOrArray, eachKeyCountOrValueTester, valueTester) {
            ...
            expectedKeys.forEach(function (count, name) {
                ...
                if (!ts.isArray(expectedKeysMapOrArray) || eachKeyCountOrValueTester !== undefined) {
                    ...
                    if (expectedValues) {
                        assert.deepEqual(actual.get(name).map(valueMapper), expectedValues.get(name), caption + ":: expected values mismatch for " + name);
                    }
                }
            });
        }
        ...
        function checkWatchedFiles(host, expectedFiles, additionalInfo) {
        ...
        function checkWatchedFilesDetailed(host, expectedFiles, eachFileWatchCountOrExpectedDetails, expectedDetails) {
            if (!ts.isNumber(eachFileWatchCountOrExpectedDetails))
                expectedDetails = eachFileWatchCountOrExpectedDetails;
            if (ts.isArray(expectedFiles)) {
                checkMap("watchedFiles", host.watchedFiles, expectedFiles, eachFileWatchCountOrExpectedDetails, [expectedDetails, function (_a) {
                        var fileName = _a.fileName, pollingInterval = _a.pollingInterval;
                        return ({ fileName: fileName, pollingInterval: pollingInterval });
                    }]);
            }
            else {
                checkMap("watchedFiles", host.watchedFiles, expectedFiles, [expectedDetails, function (_a) {
                        var fileName = _a.fileName, pollingInterval = _a.pollingInterval;
                        return ({ fileName: fileName, pollingInterval: pollingInterval });
                    }]);
            }
        ...
        function checkWatchedDirectoriesDetailed(host, expectedDirectories, recursiveOrEachDirectoryWatchCount, recursiveOrExpectedDetails, expectedDetails) {
            if (typeof recursiveOrExpectedDetails !== "boolean")
                expectedDetails = recursiveOrExpectedDetails;
            if (ts.isArray(expectedDirectories)) {
                checkMap("fsWatches" + (recursiveOrExpectedDetails ? " recursive" : ""), recursiveOrExpectedDetails ? host.fsWatchesRecursive : host.fsWatches, expectedDirectories, recursiveOrEachDirectoryWatchCount, [expectedDetails, function (_a) {
                        var directoryName = _a.directoryName, fallbackPollingInterval = _a.fallbackPollingInterval, fallbackOptions = _a.fallbackOptions;
                        return ({ directoryName: directoryName, fallbackPollingInterval: fallbackPollingInterval, fallbackOptions: fallbackOptions });
                    }]);
            }
            else {
                recursiveOrExpectedDetails = recursiveOrEachDirectoryWatchCount;
                checkMap("fsWatches" + (recursiveOrExpectedDetails ? " recursive" : ""), recursiveOrExpectedDetails ? host.fsWatchesRecursive : host.fsWatches, expectedDirectories, [expectedDetails, function (_a) {
                        var directoryName = _a.directoryName, fallbackPollingInterval = _a.fallbackPollingInterval, fallbackOptions = _a.fallbackOptions;
                        return ({ directoryName: directoryName, fallbackPollingInterval: fallbackPollingInterval, fallbackOptions: fallbackOptions });
                    }]);
            }
        ...
        var Callbacks = /** @class */ (function () {
            ...
            Callbacks.prototype.register = function (cb, args) {
                ...
                this.nextId++;
                this.map[timeoutId] = cb.bind.apply(cb, __spreadArray([/*this*/ undefined], args));
                return timeoutId;
            };
        ...
        (function (Tsc_WatchFile) {
            ...
            Tsc_WatchFile["SingleFileWatcherPerName"] = "SingleFileWatcherPerName";
        ...
        var TestServerHost = /** @class */ (function () {
            function TestServerHost(withSafeList, fileOrFolderorSymLinkList, _a) {
                var _this = this;
                ...
                var _c = ts.createSystemWatchFunctions({
                    ...
                    defaultWatchFileKind: function () { var _a; return (_a = _this.defaultWatchFileKind) === null || _a === void 0 ? void 0 : _a.call(_this); },
            ...
            TestServerHost.prototype.reloadFS = function (fileOrFolderOrSymLinkList, options) {
                ...
                var filesOrFoldersToLoad = !this.windowsStyleRoot ? fileOrFolderOrSymLinkList :
            ...
            TestServerHost.prototype.ensureFileOrFolder = function (fileOrDirectoryOrSymLink, ignoreWatchInvokedWithTriggerAsFileCreate, ignoreParentWatch) {
                if (isFile(fileOrDirectoryOrSymLink)) {
                    ...
                    if (!this.fs.get(file.path)) {
                        var baseFolder = this.ensureFolder(ts.getDirectoryPath(file.fullPath), ignoreParentWatch);
                    ...
                    var baseFolder = this.ensureFolder(ts.getDirectoryPath(symLink.fullPath), ignoreParentWatch);
                    ...
                    this.ensureFolder(ts.getDirectoryPath(fullPath), ignoreParentWatch);
                    this.ensureFolder(fullPath, ignoreWatchInvokedWithTriggerAsFileCreate);
                }
            };
            TestServerHost.prototype.ensureFolder = function (fullPath, ignoreWatch) {
                var path = this.toPath(fullPath);
                var folder = this.fs.get(path);
                if (!folder) {
                    ...
                    if (fullPath !== baseFullPath) {
                        ...
                        var baseFolder = this.ensureFolder(baseFullPath, ignoreWatch);
            ...
            TestServerHost.prototype.watchFileWorker = function (fileName, cb, pollingInterval) {
                return createWatcher(this.watchedFiles, this.toFullPath(fileName), { fileName: fileName, cb: cb, pollingInterval: pollingInterval });
            };
            TestServerHost.prototype.fsWatch = function (fileOrDirectory, _entryKind, cb, recursive, fallbackPollingInterval, fallbackOptions) {
            ...
            TestServerHost.prototype.fsWatchCallback = function (map, fullPath, eventName, entryFullPath) {
                var _this = this;
                invokeWatcherCallbacks(map.get(this.toPath(fullPath)), function (_a) {
                    var cb = _a.cb;
                    return cb(eventName, entryFullPath ? _this.getRelativePathToDirectory(fullPath, entryFullPath) : "");
                });
            };
            TestServerHost.prototype.invokeFsWatchesCallbacks = function (fullPath, eventName, entryFullPath) {
                this.fsWatchCallback(this.fsWatches, fullPath, eventName, entryFullPath);
            };
            TestServerHost.prototype.invokeFsWatchesRecursiveCallbacks = function (fullPath, eventName, entryFullPath) {
            ...
            TestServerHost.prototype.invokeRecursiveFsWatches = function (fullPath, eventName, entryFullPath) {
            ...
            TestServerHost.prototype.invokeFsWatches = function (fullPath, eventName) {
            ...
            TestServerHost.prototype.invokeFileAndFsWatches = function (fileOrFolderFullPath, eventKind) {
            ...
            TestServerHost.prototype.diff = function (baseline, base) {
                ...
                this.fs.forEach(function (newFsEntry) {
                    diffFsEntry(baseline, base.get(newFsEntry.path), newFsEntry, _this.writtenFiles);
                });
                base.forEach(function (oldFsEntry) {
                    var newFsEntry = _this.fs.get(oldFsEntry.path);
                    if (!newFsEntry) {
                        diffFsEntry(baseline, oldFsEntry, newFsEntry, _this.writtenFiles);
                    }
                });
                baseline.push("");
            };
            TestServerHost.prototype.serializeWatches = function (baseline) {
                serializeMultiMap(baseline, "WatchedFiles", this.watchedFiles, function (_a) {
                    var fileName = _a.fileName, pollingInterval = _a.pollingInterval;
                    return ({ fileName: fileName, pollingInterval: pollingInterval });
                });
                baseline.push("");
        ...
        function diffFsFile(baseline, fsEntry) {
            baseline.push("//// [" + fsEntry.fullPath + "]\r\n" + fsEntry.content, "");
        }
        function diffFsSymLink(baseline, fsEntry) {
            baseline.push("//// [" + fsEntry.fullPath + "] symlink(" + fsEntry.symLink + ")");
        }
        function diffFsEntry(baseline, oldFsEntry, newFsEntry, writtenFiles) {
        ...
        function serializeTestFsWatcher(_a) {
            var directoryName = _a.directoryName, fallbackPollingInterval = _a.fallbackPollingInterval, fallbackOptions = _a.fallbackOptions;
            return {
                directoryName: directoryName,
                fallbackPollingInterval: fallbackPollingInterval,
        ...
        function serializeWatchOptions(fallbackOptions) {
            if (!fallbackOptions)
                return undefined;
            var watchFile = fallbackOptions.watchFile, watchDirectory = fallbackOptions.watchDirectory, fallbackPolling = fallbackOptions.fallbackPolling, rest = __rest(fallbackOptions, ["watchFile", "watchDirectory", "fallbackPolling"]);
            return __assign({ watchFile: watchFile !== undefined ? ts.WatchFileKind[watchFile] : undefined, watchDirectory: watchDirectory !== undefined ? ts.WatchDirectoryKind[watchDirectory] : undefined, fallbackPolling: fallbackPolling !== undefined ? ts.PollingWatchKind[fallbackPolling] : undefined }, rest);
        }
        function serializeMultiMap(baseline, caption, multiMap, valueMapper) {
...
(function (FourSlash) {
    ...
    var TestState = /** @class */ (function () {
                ...
                if (!compilationOptions.noLib) {
                    ...
                    var addSourceFile_1 = function (fileName) {
                        ...
                        for (var _i = 0, _a = libFile.libReferenceDirectives; _i < _a.length; _i++) {
                            var directive = _a[_i];
                            addSourceFile_1("lib." + directive.fileName + ".d.ts");
                        }
                    };
        ...
        TestState.prototype.verifyOrganizeImports = function (newContent) {
            var changes = this.languageService.organizeImports({ fileName: this.activeFile.fileName, type: "file" }, this.formatCodeSettings, ts.emptyOptions);
        ...
        TestState.prototype.getDiagnostics = function (fileName, includeSuggestions) {
            ...
            return __spreadArray(__spreadArray(__spreadArray([], this.languageService.getSyntacticDiagnostics(fileName)), this.languageService.getSemanticDiagnostics(fileName)), (includeSuggestions ? this.languageService.getSuggestionDiagnostics(fileName) : ts.emptyArray));
        };
        ...
        TestState.prototype.verifyNoErrors = function () {
            ...
            ts.forEachKey(this.inputFiles, function (fileName) {
                if (!ts.isAnySupportedFileExtension(fileName)
                    ...
                    || !ts.getAllowJSCompilerOption(_this.getProgram().getCompilerOptions()) && !ts.resolutionExtensionIsTSOrJson(ts.extensionFromPath(fileName)))
                    return;
        ...
        TestState.prototype.verifyGoToXWorker = function (startMarker, endMarkers, getDefs, startMarkerName) {
            ...
            ts.zipWith(endMarkers, definitions, function (endMarkerOrFileResult, definition, i) {
                ...
                if (ts.comparePaths(expectedFileName, definition.fileName, /*ignoreCase*/ true) !== 0 /* EqualTo */ || expectedPosition !== definition.textSpan.start) {
                    var filesToDisplay = ts.deduplicate([expectedFileName, definition.fileName], ts.equateValues);
                    var markers_1 = [{ text: "EXPECTED", fileName: expectedFileName, position: expectedPosition }, { text: "ACTUAL", fileName: definition.fileName, position: definition.textSpan.start }];
                    var text = filesToDisplay.map(function (fileName) {
                        var markersToRender = markers_1.filter(function (m) { return m.fileName === fileName; }).sort(function (a, b) { return b.position - a.position; });
                        ...
                        for (var _i = 0, markersToRender_1 = markersToRender; _i < markersToRender_1.length; _i++) {
                            var marker_1 = markersToRender_1[_i];
                            fileContent = fileContent.slice(0, marker_1.position) + ("\u001B[1;4m/*" + marker_1.text + "*/\u001B[0;31m") + fileContent.slice(marker_1.position);
                        }
                        return "// @Filename: " + fileName + "\n" + fileContent;
                    }).join("\n\n");
                    _this.raiseError(testName + " failed for definition " + (markerName || expectedFileName) + " (" + i + "): expected " + expectedFileName + " at " + expectedPosition + ", got " + definition.fileName + " at " + definition.textSpan.start + "\n\n" + text + "\n");
                }
                if (definition.unverified && (typeof endMarkerOrFileResult === "string" || !endMarkerOrFileResult.unverified)) {
                    ...
                    _this.raiseError(testName + " failed for definition " + (markerName || expectedFileName) + " (" + i + "): The actual definition was an `unverified` result. Use:\n\n" +
        ...
        TestState.prototype.verifyDefinitionTextSpan = function (defs, startMarkerName) {
            ...
            if (!range) {
                ...
                var spanContentWithMarker = spanContent.slice(0, marker.position - defs.textSpan.start) + ("/*" + startMarkerName + "*/") + spanContent.slice(marker.position - defs.textSpan.start);
                var suggestedFileContent = (fileContent.slice(0, defs.textSpan.start) + ("\u001B[1;4m[|" + spanContentWithMarker + "|]\u001B[0;31m") + fileContent.slice(ts.textSpanEnd(defs.textSpan)))
        ...
        TestState.prototype.verifyCompletionsWorker = function (options) {
            ...
            var _loop_9 = function (entry) {
                ...
                if (!entries) {
                    ...
                    if (entries.some(function (e) { return e.source === entry.source && _this.deepEqual(e.data, entry.data); })) {
            ...
            if (ts.hasProperty(options, "exact")) {
                ...
                if (options.includes) {
                    for (var _b = 0, _c = toArray(options.includes); _b < _c.length; _b++) {
                        ...
                        if (!found)
                            throw this.raiseError("Includes: completion '" + name + "' not found.");
                        if (!found.length)
                            throw this.raiseError("Includes: no completions with name '" + name + "' remain unmatched.");
        ...
        TestState.prototype.verifyCompletionEntry = function (actual, expected) {
            ...
            if (expected.isFromUncheckedFile !== undefined) {
                assert.equal(actual.isFromUncheckedFile, expected.isFromUncheckedFile, "Expected 'isFromUncheckedFile' properties to match");
            }
            if (expected.isPackageJsonImport !== undefined) {
                assert.equal(actual.isPackageJsonImport, expected.isPackageJsonImport, "Expected 'isPackageJsonImport' properties to match");
            }
            ...
            if (expected.text !== undefined) {
                var actualDetails = ts.Debug.checkDefined(this.getCompletionEntryDetails(actual.name, actual.source, actual.data), "No completion details available for name '" + actual.name + "' and source '" + actual.source + "'");
        ...
        TestState.prototype.verifyCompletionsAreExactly = function (actual, expected, marker) {
            ...
            ts.zipWith(actual, expected, function (completion, expectedCompletion, index) {
                ...
                if (completion.name !== name) {
                    _this.raiseError((marker ? JSON.stringify(marker) : "") + " Expected completion at index " + index + " to be " + name + ", got " + completion.name);
                }
        ...
        TestState.prototype.verifySymbol = function (symbol, declarationRanges) {
            ...
            if ((declarations === null || declarations === void 0 ? void 0 : declarations.length) !== declarationRanges.length) {
                this.raiseError("Expected to get " + declarationRanges.length + " declarations, got " + (declarations === null || declarations === void 0 ? void 0 : declarations.length));
            }
        ...
        TestState.prototype.symbolsInScope = function (range) {
            ...
            return this.getChecker().getSymbolsInScope(node, 111551 /* Value */ | 788968 /* Type */ | 1920 /* Namespace */);
        };
        ...
        TestState.prototype.verifyDocumentHighlightsRespectFilesList = function (files) {
            ...
            var _loop_10 = function (fileName) {
        ...
        TestState.prototype.verifyReferenceGroups = function (starts, parts) {
            ...
            var fullExpected = ts.map(parts, function (_a) {
                ...
                return ({
                    definition: typeof definition === "string" ? definition : __assign(__assign({}, definition), { range: ts.createTextSpanFromRange(definition.range) }),
                    references: ranges.map(function (r) {
                        var _a = (r.marker && r.marker.data || {}), _b = _a.isWriteAccess, isWriteAccess = _b === void 0 ? false : _b, _c = _a.isDefinition, isDefinition = _c === void 0 ? false : _c, isInString = _a.isInString, contextRangeIndex = _a.contextRangeIndex, contextRangeDelta = _a.contextRangeDelta, contextRangeId = _a.contextRangeId;
                        var contextSpan;
                        if (contextRangeDelta !== undefined) {
                            var allRanges = _this.getRanges();
                            var index = allRanges.indexOf(r);
                            if (index !== -1) {
                        ...
                        return __assign(__assign(__assign({ textSpan: ts.createTextSpanFromRange(r), fileName: r.fileName }, (contextSpan ? { contextSpan: contextSpan } : undefined)), { isWriteAccess: isWriteAccess, isDefinition: isDefinition }), (isInString ? { isInString: true } : undefined));
                    }),
                });
            });
        ...
        TestState.prototype.verifyNoReferences = function (markerNameOrRange) {
            if (markerNameOrRange !== undefined)
        ...
        TestState.prototype.assertObjectsEqual = function (fullActual, fullExpected, msgPrefix) {
            ...
            var recur = function (actual, expected, path) {
                var fail = function (msg) {
                    _this.raiseError(msgPrefix + " At " + path + ": " + msg + " " + displayExpectedAndActualString(stringify(fullExpected), stringify(fullActual)));
                };
        ...
        TestState.prototype.deepEqual = function (a, b) {
            try {
        ...
        TestState.prototype.verifyRenameLocations = function (startRanges, options) {
            ...
            for (var _i = 0, _startRanges_1 = _startRanges; _i < _startRanges_1.length; _i++) {
                ...
                assert.deepEqual(sort(references), sort(ranges.map(function (rangeOrOptions) {
                    var _a = "range" in rangeOrOptions ? rangeOrOptions : { range: rangeOrOptions }, range = _a.range, prefixSuffixText = __rest(_a, ["range"]); // eslint-disable-line no-in-operator
        ...
        TestState.prototype.baselineGetEmitOutput = function () {
            ...
            Harness.Baseline.runBaseline(ts.Debug.checkDefined(this.testData.globalOptions["baselinefile" /* baselineFile */]), resultString);
        };
        ...
        TestState.prototype.getSyntacticDiagnosticBaselineText = function (files) {
            ...
            var result = "Syntactic Diagnostics for file '" + this.originalInputFileName + "':"
                + Harness.IO.newLine()
        ...
        TestState.prototype.getSemanticDiagnosticBaselineText = function (files) {
            ...
            var result = "Semantic Diagnostics for file '" + this.originalInputFileName + "':"
                + Harness.IO.newLine()
                + Harness.Compiler.getErrorBaseline(files, diagnostics, /*pretty*/ false);
            return result;
        };
        ...
        TestState.prototype.getBaselineFileNameForInternalFourslashFile = function (ext) {
            ...
            return this.testData.globalOptions["baselinefile" /* baselineFile */] ||
                ts.getBaseFileName(this.activeFile.fileName).replace(".ts" /* Ts */, ext);
        };
        TestState.prototype.getBaselineFileNameForContainingTestFile = function (ext) {
            if (ext === void 0) { ext = ".baseline"; }
            return ts.getBaseFileName(this.originalInputFileName).replace(".ts" /* Ts */, ext);
        };
        ...
        TestState.prototype.checkPostEditInvariants = function () {
            ...
            var referenceSourceFile = ts.createLanguageServiceSourceFile(this.activeFile.fileName, createScriptSnapShot(content), 99 /* Latest */, /*version:*/ "0", /*setNodeParents:*/ false);
        ...
        TestState.prototype.baselineCurrentFileNameOrDottedNameSpans = function () {
            var _this = this;
            Harness.Baseline.runBaseline(this.testData.globalOptions["baselinefile" /* baselineFile */], this.baselineCurrentFileLocations(function (pos) { return _this.getNameOrDottedNameSpan(pos); }));
        };
        ...
        TestState.prototype.classificationToIdentifier = function (classification) {
            ...
            return __spreadArray([tokenTypes[typeIdx]], tokenModifiers.filter(function (_, i) { return modSet & 1 << i; })).join(".");
        };
        ...
        TestState.prototype.verifyOutliningHintSpans = function (spans) {
            ...
            ts.zipWith(spans, actual, function (expectedSpan, actualSpan, i) {
                if (expectedSpan.pos !== actualSpan.hintSpan.start || expectedSpan.end !== ts.textSpanEnd(actualSpan.hintSpan)) {
                    return _this.raiseError("verifyOutliningSpans failed - span " + (i + 1) + " expected: (" + expectedSpan.pos + "," + expectedSpan.end + "),  actual: (" + actualSpan.hintSpan.start + "," + ts.textSpanEnd(actualSpan.hintSpan) + ")");
                }
            });
        };
        ...
        TestState.prototype.applyCodeActionFromCompletion = function (markerName, options) {
            ...
            if (!details) {
                ...
                var detailMessage = (matchingName === null || matchingName === void 0 ? void 0 : matchingName.length)
                    ? "\n  Found " + matchingName.length + " with name '" + options.name + "' from source(s) " + matchingName.map(function (e) { return "'" + e.source + "'"; }).join(", ") + "."
            ...
            if ((codeActions === null || codeActions === void 0 ? void 0 : codeActions.length) !== 1) {
                this.raiseError("Expected one code action, got " + ((_b = codeActions === null || codeActions === void 0 ? void 0 : codeActions.length) !== null && _b !== void 0 ? _b : 0));
            }
        ...
        TestState.prototype.verifyCodeFixAll = function (_a) {
            ...
            ts.Debug.assert(fixWithId !== undefined, "No available code fix has the expected id. Fix All is not available if there is only one potentially fixable diagnostic present.", function () {
                return "Expected '" + fixId + "'. Available actions:\n" + ts.mapDefined(_this.getCodeFixes(_this.activeFile.fileName), function (a) { return a.fixName + " (" + (a.fixId || "no fix id") + ")"; }).join("\n");
            });
        ...
        TestState.prototype.verifyNewContent = function (_a, changes) {
            ...
            if (newRangeContent !== undefined) {
                ...
                var _loop_13 = function (newFileName) {
        ...
        TestState.prototype.verifyImportFixModuleSpecifiers = function (markerName, moduleSpecifiers) {
            var marker = this.getMarkerByName(markerName);
            var codeFixes = this.getCodeFixes(marker.fileName, ts.Diagnostics.Cannot_find_name_0.code, {
                includeCompletionsForModuleExports: true,
                includeCompletionsWithInsertText: true
            }, marker.position).filter(function (f) { return f.fixName === ts.codefix.importFixName; });
        ...
        TestState.prototype.verifyNavigateTo = function (options) {
            for (var _i = 0, options_1 = options; _i < options_1.length; _i++) {
                ...
                this.assertObjectsEqual(items, expected.map(function (e) { return ({
                    ...
                    containerKind: e.containerKind || "" /* unknown */,
                }); }));
            }
        };
        ...
        TestState.prototype.verifyDocumentHighlights = function (expectedRanges, fileNames) {
            ...
            var _loop_14 = function (fileName) {
                ...
                if (expectedRangesInFile.length !== spansInFile.length) {
                    this_3.raiseError("verifyDocumentHighlights failed - In " + fileName + ", expected " + expectedRangesInFile.length + " highlights, got " + spansInFile.length);
                }
        ...
        TestState.prototype.verifyCodeFixAllAvailable = function (negative, fixName) {
            ...
            if (negative && hasFix) {
                ...
                this.raiseError("Expected to find a fix with the name '" + fixName + "', but none exists." +
                    availableFixes.length
                    ? " Available fixes: " + availableFixes.map(function (fix) { return fix.fixName + " (" + (fix.fixId ? "with" : "without") + " fix-all)"; }).join(", ")
                    : "");
            }
        };
        ...
        TestState.prototype.verifyRefactorAvailable = function (negative, triggerReason, name, actionName) {
        ...
        TestState.prototype.testNewFileContents = function (edits, newFileContents, description) {
            ...
            var _loop_15 = function (fileName) {
        ...
        TestState.prototype.formatCallHierarchyItem = function (file, callHierarchyItem, direction, seen, prefix, trailingPrefix) {
            if (trailingPrefix === void 0) { trailingPrefix = prefix; }
            var key = callHierarchyItem.file + "|" + JSON.stringify(callHierarchyItem.span) + "|" + direction;
            var alreadySeen = seen.has(key);
            seen.set(key, true);
            var incomingCalls = direction === 2 /* Outgoing */ ? { result: "skip" } :
            ...
            var outgoingCalls = direction === 1 /* Incoming */ ? { result: "skip" } :
                alreadySeen ? { result: "seen" } :
            ...
            text += this.formatCallHierarchyItemSpan(file, callHierarchyItem.selectionSpan, prefix + "\u2502 ", incomingCalls.result !== "skip" || outgoingCalls.result !== "skip" ? prefix + "\u2502 " :
                trailingPrefix + "\u2570 ");
            if (incomingCalls.result === "seen") {
                ...
                if (!ts.some(incomingCalls.values)) {
                    ...
                    for (var i = 0; i < incomingCalls.values.length; i++) {
                        ...
                        text += prefix + "\u2502 \u256D from:\n";
                        text += this.formatCallHierarchyItem(file_1, incomingCall.from, 1 /* Incoming */, seen, prefix + "\u2502 \u2502 ");
                        text += prefix + "\u2502 \u251C fromSpans:\n";
                        text += this.formatCallHierarchyItemSpans(file_1, incomingCall.fromSpans, prefix + "\u2502 \u2502 ", i < incomingCalls.values.length - 1 ? prefix + "\u2502 \u2570 " :
            ...
            if (outgoingCalls.result === "seen") {
                ...
                if (!ts.some(outgoingCalls.values)) {
                    ...
                    for (var i = 0; i < outgoingCalls.values.length; i++) {
                        ...
                        text += prefix + "\u2502 \u256D to:\n";
                        text += this.formatCallHierarchyItem(this.findFile(outgoingCall.to.file), outgoingCall.to, 2 /* Outgoing */, seen, prefix + "\u2502 \u2502 ");
                        text += prefix + "\u2502 \u251C fromSpans:\n";
                        text += this.formatCallHierarchyItemSpans(file, outgoingCall.fromSpans, prefix + "\u2502 \u2502 ", i < outgoingCalls.values.length - 1 ? prefix + "\u2502 \u2570 " :
                            trailingPrefix + "\u2570 \u2570 ");
                    }
                }
            }
        ...
        TestState.prototype.formatCallHierarchy = function (callHierarchyItem) {
            ...
            if (callHierarchyItem) {
                ...
                text += this.formatCallHierarchyItem(file, callHierarchyItem, 0 /* Root */, new ts.Map(), "");
            }
            return text;
        };
    ...
    function runFourSlashTestContent(basePath, testType, content, fileName) {
        ...
        var output = ts.transpileModule(content, { reportDiagnostics: true, fileName: actualFileName, compilerOptions: { target: 2 /* ES2015 */, inlineSourceMap: true, inlineSources: true } });
    ...
    function runCode(code, state, fileName) {
        ...
        var wrappedCode = "(function(test, goTo, plugins, verify, edit, debug, format, cancellation, classification, completion, verifyOperationIsCancelled) {" + code + "\n//# sourceURL=" + ts.getBaseFileName(generatedFile) + "\n})";
    ...
    function getNonFileNameOptionInObject(optionObject) {
        for (var option in optionObject) {
            switch (option) {
                case "filename" /* fileName */:
                case "baselinefile" /* baselineFile */:
                case "emitthisfile" /* emitThisFile */:
                    break;
                default:
                    return option;
            }
...
(function (FourSlashInterface) {
    ...
    var Verify = /** @class */ (function (_super) {
        ...
        Verify.prototype.baselineCompletions = function () {
        ...
        Verify.prototype.importFixModuleSpecifiers = function (marker, moduleSpecifiers) {
        ...
        Verify.prototype.organizeImports = function (newContent) {
    ...
    var Completion;
    (function (Completion) {
        Completion.SortText = ts.Completions.SortText;
        ...
        Completion.typeKeywords = ["false", "null", "true", "void", "asserts", "any", "boolean", "infer", "keyof", "never", "readonly", "number", "object", "string", "symbol", "undefined", "unique", "unknown", "bigint"].map(keywordEntry);
        var globalTypeDecls = [
        ...
        function globalTypesPlus(plus) {
            return __spreadArray(__spreadArray(__spreadArray([
                Completion.globalThisEntry
            ], globalTypeDecls), plus), Completion.typeKeywords);
        }
        ...
        Completion.classElementKeywords = ["private", "protected", "public", "static", "abstract", "async", "constructor", "declare", "get", "readonly", "set", "override"].map(keywordEntry);
        Completion.classElementInJsKeywords = getInJsKeywords(Completion.classElementKeywords);
        Completion.constructorParameterKeywords = ["private", "protected", "public", "readonly", "override"].map(function (name) { return ({
            name: name,
            kind: "keyword",
            sortText: Completion.SortText.GlobalsOrKeywords
        }); });
        ...
        Completion.functionMembersWithPrototype = __spreadArray(__spreadArray(__spreadArray([], Completion.functionMembers.slice(0, 4)), [
        ...
        Completion.globalsInsideFunction = function (plus) { return __spreadArray(__spreadArray(__spreadArray(__spreadArray(__spreadArray([
        ...
        // TODO: many of these are inappropriate to always provide
        Completion.globalsInJsInsideFunction = function (plus) { return __spreadArray(__spreadArray(__spreadArray(__spreadArray([
            { name: "arguments", kind: "local var" },
            Completion.globalThisEntry
        ], Completion.globalsVars), plus), [
            Completion.undefinedVarEntry
                        ...
                        return __generator(this, function (_b) {
                            switch (_b.label) {
                                ...
                                case 1:
                                    ...
                                    ts.forEachChild(elem, function (c) { resChildren.unshift(c); });
    ...
    var TypeWriterWalker = /** @class */ (function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        node_2 = res.value;
                        if (!(ts.isExpressionNode(node_2) || node_2.kind === 78 /* Identifier */ || ts.isDeclarationName(node_2))) return [3 /*break*/, 3];
        ...
        TypeWriterWalker.prototype.writeTypeOrSymbol = function (node, isSymbolWalk) {
            ...
            if (!isSymbolWalk) {
                ...
                if (ts.isPartOfTypeNode(node) || ts.isIdentifier(node) && !(ts.getMeaningFromDeclaration(node.parent) & 1 /* Value */) && !(ts.isTypeAliasDeclaration(node.parent) && node.parent.name === node)) {
                    return undefined;
                }
                ...
                (!this.hadErrorBaseline &&
                    ...
                    type.intrinsicName :
                    this.checker.typeToString(type, node.parent, 1 /* NoTruncation */ | 1048576 /* AllowUniqueESSymbolType */);
                return {
