...
export interface CompilerHost {
    ...
    files?: string[];
    exitStatus: number;
    compileWithChanges: (filesChanged: FileChanges, sourceMap?: boolean) => CompilerHost;
    messages?: string[];
}
...
let cachedExistingFiles: Record<string, boolean>
...
export class Compiler {
    ...
    public compile(options: ts.CompilerOptions, rootFileNames: string[]): CompilerHost {
    ...
    private doCompile(): CompilerHost {
        cachedExistingFiles = {}
    ...
    parseTsconfig(projectRoot: string, isPublish: boolean) {
        ...
        try {
            ...
            console.log('Warning! The tsconfig.json is not a valid json.')//不是有效的 json 文件
            // errLog.push();
            configObj = {
                "compilerOptions": {
                    "target": "es5",
        ...
        for (let optionName of notSupport) {
            if (compilerOptions.hasOwnProperty(optionName)) {
                var error = `Not support the compiler option: '${optionName}' in tsconfig.json.`;//这个编译选项目前不支持修改
